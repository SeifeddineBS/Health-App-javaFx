...
@Override
 ...
 @Override
    ...
    {  try {
            String requete  = "INSERT INTO `participationactivte` (`id_client`,`id_activite`) VALUES ('"+c+"','"+a.getId()+"') ";
            Statement st =Connexion.getInstance().getConnection().createStatement() ;
            st.executeUpdate(requete);
...
public void updaterating(Activites a,int rate,String i){
    try {
         String requete = " UPDATE participationactivte SET rating='"+rate+"' WHERE id_activite='"+a.getId()+"' And id_client='"+i+"'" ;
...
public int sommerating(int id){
    int i=0;
    try {
         String requete = " SELECT count(id_activite) avg(rating) FROM participationactivte  WHERE id_activite='"+id+"'" ;
            Statement st =Connexion.getInstance().getConnection().createStatement();
            ResultSet rs = st.executeQuery(requete);
    ...
    public ObservableList<Activites> showActivitesClient(String Ac) {
        ...
        try {
            ...
            String requete = "SELECt description,date From activite A JOIN participationactivte p ON A.id=p.id_activite Where p.id_client='"+Ac+"' AND date='4/2/2021'";
                       // String requete = "SELECt description,date From activite A JOIN participationactivte p ON A.id=p.id_activite Where p.id_client='"+Ac+"' AND date=DATE_FORMAT(SYSDATE(), '%m/%d/%Y')";
