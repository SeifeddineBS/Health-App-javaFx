...
public class Ajouter_AdminController implements Initializable {
    ...
    @FXML
    private PasswordField password_a;
    @FXML
    private PasswordField c_password_a;
    @FXML
    private TextField tel_a;
    @FXML
    private Label Confirmer_MotDePasse_label_a;
    @FXML
    private Label test_mail_label_a;
    @FXML
    private Label verif_id_bd_label_a;
    @FXML
    private Label verif_tel_label_a;
    @FXML
    ...
    @FXML
    private Text verif_mdp_label_a;
    ...
    @FXML
    private void ajouter_admin(ActionEvent event) throws Exception {
        ServiceUser su = new ServiceUser();
        ServiceAdmin sa = new ServiceAdmin();
        SendMail sm = new SendMail();

        String id = id_a.getText();
        String tel = tel_a.getText();
        String email = email_a.getText();
        String nom = nom_a.getText();
        String prenom = prenom_a.getText();
        String password = password_a.getText();
        String c_password = c_password_a.getText();
        ...
        if (!su.test_Cin(id)) {
            ...
            a.setEmail(email);
            a.setTel(tel);
            a.setPassword(su.crypter_password(password));
            a.setPicture(picture);
            sa.ajouterAdmin(a);
            load_data();
            information_Box("Compte Crée", "Un admin ajouté avec l'identifiant " + id);
            sm.envoyerMail(email, "Salut " + nom, "Vous Avez ete ajouté comme admin Votre Identifiant est: " + id + "et votre mot de passe est:" + password);

        }
    }

    @FXML
    private void Confirmer_MotDePasse_a(KeyEvent event) {

        String password = password_a.getText();
        String c_password = c_password_a.getText();

        if (!c_password.equals(password)) {
            Confirmer_MotDePasse_label_a.setText("Verifier Votre Mot De Passe");

        } else {
            Confirmer_MotDePasse_label_a.setText("");
    ...
    @FXML
    private void test_Mail_a(KeyEvent event) {
        ServiceUser su = new ServiceUser();
        ...
        if (!su.test_Email(email)) {
            test_mail_label_a.setText("Verifier Votre Adresse Email");
        } else {
            test_mail_label_a.setText("");
        }
    ...
    @FXML
    private void verif_id_bd_a(KeyEvent event) {
        ServiceUser su = new ServiceUser();

        String id = id_a.getText();
        boolean test = su.verifier_id_bd(id);
        if (test) {
            verif_id_bd_label_a.setText("Le Cin Existe Deja !");
        } else {
            verif_id_bd_label_a.setText("");
    ...
    private void verif_password_a(KeyEvent event) {
        ...
        if (!test) {
            verif_mdp_label_a.setText("votre mot de passe doit contenir au minimum 8 caractères\n à savoir : au moins une lettre minuscule,une lettre majuscule et un chiffre ");

        } else {
            verif_mdp_label_a.setText("");
    ...
    @FXML
    private void verif_tel_a(KeyEvent event) {
        String tel = tel_a.getText();
        ...
        if (!test) {
            verif_tel_label_a.setText("Numero non  valide");
        } else {
            verif_tel_label_a.setText("");
