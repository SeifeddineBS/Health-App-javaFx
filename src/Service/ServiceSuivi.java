...
public class ServiceSuivi implements IServiceSuivi {
    ...
    public void supprimerObjectif(int id) {
        try {
            ...
            st.executeUpdate(query);
            System.out.println("suppression suivi avec succes");
        } catch (SQLException ex) {
            System.out.println("erreur supprimer suivi");
            System.out.println(ex);
        }
    }
    ...
    @Override
    public ObservableList<Suivi> getSuiviparid(int id) {
        ObservableList<Suivi> suivis = FXCollections.observableArrayList();
        ServiceClient sc = new ServiceClient();
        ServiceObjectif so = new ServiceObjectif();
        ...
        try {
            Statement st = cnx.createStatement();
            String query = "select * from `suivi` WHERE idObjectif LIKE '" + id + "'";
            ResultSet rs = st.executeQuery(query);
            while (rs.next()) {
                Suivi ss = new Suivi();
                ss.setIdSuiv(rs.getInt(1));
