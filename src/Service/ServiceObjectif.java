...
public class ServiceObjectif implements IServiceObjectif {
    ...
    public void ajouterObjectif(Objectif o, int checked) {
        ...
        try {
            ...
            String query = "INSERT INTO `objectif`(`id`, `description`, `reponse`, `dateDebut`, `duree`, `idClient`, `mailchecked`, image) "
                    + "VALUES ('" + o.getId() + "','"
                    + o.getDescription() + "','"
                    + o.getReponse() + "','"
                    + date + "','"
    ...
    public List<Objectif> trierObjectifparDate() {
        ...
        try {
            Statement st = cnx.createStatement();
            String query = "select * from `objectif` ORDER BY dateDebut";
            ResultSet rs = st.executeQuery(query);
            while (rs.next()) {
                Objectif o = new Objectif();
                o.setId(rs.getInt(1));
